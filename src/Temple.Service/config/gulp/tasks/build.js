...
gulp.task('build-prod', function (done) {
    runSequence('clean-build', 'tsc', 'build-systemjs', 'build-assets', done);
});


gulp.task('build-release', ['clean-release', 'build'], function (done) {
    gulp.src(config.build.path + '/**/*')
    .pipe(gulp.dest(config.build.releasePath))
    .on('finish', done);
});
...
gulp.task('build-assets', function (done) {
    runSequence('clean-build', ['sass', 'fonts'], function () {
        ...
        gulp.src([config.index, config.callback, config.silentcallback], { base: config.src })
            .pipe(useref())
            .pipe(gulpif('assets/lib.js', uglify()))
            .pipe(gulpif('assets/callbacklib.js', uglify()))
            .pipe(gulpif('assets/silentcallbacklib.js', uglify()))
